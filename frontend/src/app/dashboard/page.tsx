...
import { 
  LogOut, Brain, Target, Trophy, Timer, Flame, Sparkles, 
  BookOpen, User, Calendar, CheckCircle, XCircle, Edit3, Save, X
} from "lucide-react";
...
export default function Dashboard() {
  ...
  const features = [
    ...
    { title: "Leaderboards", icon: Trophy, color: "from-yellow-500 to-amber-600", href: "/leaderboard" },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 text-white relative">
      ...
      {showProfile && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-50 flex items-center justify-center p-6" onClick={() => { setShowProfile(false); setIsEditing(false); }}>
          <div className="bg-gradient-to-br from-gray-900 to-black border border-white/20 rounded-3xl p-10 max-w-2xl w-full relative" onClick={(e) => e.stopPropagation()}>
            
            <button onClick={() => { setShowProfile(false); setIsEditing(false); }} className="absolute top-6 right-6 text-gray-400 hover:text-white">
              <X className="w-8 h-8" />
            </button>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                {isEditing ? (
                  <Input value={editForm.phone} onChange={(e) => setEditForm({...editForm, phone: e.target.value})}
                    placeholder="9876543210" className="mt-2 bg-white/10 border-white/20 text-white" />
                ) : (
              ...
              <div>
                ...
                {isEditing ? (
                  <select value={editForm.targetExam} onChange={(e) => setEditForm({...editForm, targetExam: e.target.value})}
                    className="mt-2 w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white">
                    <option value="JEE">JEE Main + Advanced</option>
                    <option value="NEET">NEET</option>
                    <option value="Both">JEE + NEET Both</option>
                  ...
                  <p className="text-2xl font-bold text-cyan-300 mt-2">
                    {user.exam === "JEE" ? "JEE Main + Advanced" : user.exam === "NEET" ? "NEET" : user.exam}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                {isEditing ? (
                  <select value={editForm.targetYear} onChange={(e) => setEditForm({...editForm, targetYear: e.target.value})}
                    className="mt-2 w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white">
      ...
      {/* Feature Cards */}
      <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 pb-32">
        {features.map((item, i) => (
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-black/60 backdrop-blur-xl border-t border-white/10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-around text-center">
          <div><Flame className="w-8 h-8 text-orange-500 mx-auto mb-1" /><p className="text-2xl font-bold">{streak}</p><p className="text-sm text-gray-400">Streak</p></div>
          <div><CheckCircle className="w-8 h-8 text-cyan-500 mx-auto mb-1" /><p className="text-2xl font-bold">{totalMocks}</p><p className="text-sm text-gray-400">Mocks</p></div>
          <div><Target className="w-8 h-8 text-emerald-500 mx-auto mb-1" /><p className="text-2xl font-bold">{accuracy}%</p><p className="text-sm text-gray-400">Accuracy</p></div>
        </div>
      </div>
    </div>
  );
